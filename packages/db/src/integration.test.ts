...
describe("SQLite/libSQL integration", () => {
  ...
  it("audits incompatible legacy rows before migration without mutating them", async () => {
    const directory = await temporaryDirectory();
    const legacyMigrations = resolve(directory, "migrations-0007");
    await cp(migrationsFolder, legacyMigrations, { recursive: true });
    await rm(resolve(legacyMigrations, "0008_familiar_salo.sql"));
    await rm(resolve(legacyMigrations, "meta/0008_snapshot.json"));
  ...
  it("preserves compatible rows through genealogy migration and rollback", async () => {
    const directory = await temporaryDirectory();
    const legacyMigrations = resolve(directory, "migrations-0007-compatible");
    await cp(migrationsFolder, legacyMigrations, { recursive: true });
    await rm(resolve(legacyMigrations, "0008_familiar_salo.sql"));
    await rm(resolve(legacyMigrations, "meta/0008_snapshot.json"));
    const journalPath = resolve(legacyMigrations, "meta/_journal.json");
    ...
    try {
      ...
      const rollback = await readFile(resolve(packageRoot, "drizzle/rollback/0008_genealogy_integrity.sql"), "utf8");
