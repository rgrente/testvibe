...
const mocks = vi.hoisted(() => ({
  ...
  formatGenealogicalDateInput: vi.fn((value: string) => value === "1950-03" ? "mars 1950" : value),
}));
...
describe("UnionsPage", () => {
  it("relit et soumet un intervalle qualifié", async () => {
    ...
    mocks.adminListUnions.mockResolvedValue([{ id: 3, type: "mariage", startDate: "1950-03", endDate: null, place: null, latitude: null, longitude: null, personIds: [1, 2] }]);
    render(await UnionsPage({ searchParams: Promise.resolve({}) }));
    expect(screen.getByText(/depuis mars 1950/)).toBeInTheDocument();
